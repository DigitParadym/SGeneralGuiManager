...
from PySide6.QtWidgets import (
    ...
    QSplitter, QGroupBox, QLabel, QLineEdit, QPushButton, QTextEdit,
    QListWidget, QListWidgetItem, QFileDialog, QMessageBox,
    QProgressBar, QStatusBar, QTabWidget, QFrame, QComboBox,
    QCheckBox, QTreeWidget, QTreeWidgetItem, QScrollArea
)
...
class ASTMainWindow(QMainWindow):
    ...
    def browse_plan_file(self):
        file_path, _ = QFileDialog.getOpenFileName(self, "Selectionner un plan JSON", "", "Fichiers JSON (*.json)")
        if file_path:
    ...
    def add_target_files(self):
        files, _ = QFileDialog.getOpenFileNames(self, "Selectionner fichiers Python", "", "Fichiers Python (*.py)")
        if files:
    ...
    def execute_transformations(self):
        ...
        reply = QMessageBox.question(self, "Confirmation", f"Executer le plan sur {len(self.target_files)} fichier(s)?")
        if reply == QMessageBox.Yes:
            self.start_transformations()

    def start_transformations(self):
        self.worker = TransformationWorker(self.orchestrateur, self.plan_line_edit.text(), self.target_files)
    ...
    def on_file_selection_changed(self, current_item, previous_item):
        if current_item:
            ...
            full_path = next((path for path in self.target_files if os.path.basename(path) == filename), None)
            if full_path:
