...
class BowlerUtils:
            ...
            for node in ast.walk(tree):
                if isinstance(node, ast.FunctionDef):
                    analysis['functions'].append({
                        ...
                        'decorators': [BowlerUtils._get_decorator_name(d) for d in node.decorator_list],
                    ...
                    analysis['classes'].append({
                        'name': node.name,
                        'line': node.lineno,
                        'bases': [BowlerUtils._get_node_name(base) for base in node.bases],
                        'decorators': [BowlerUtils._get_decorator_name(d) for d in node.decorator_list],
                    ...
                    if hasattr(node, 'lineno') and node.lineno < 50:  # Heuristique pour detecter les globales
    ...
    @staticmethod
    def create_batch_script(opportunities: List[Dict[str, Any]], 
