...
class BowlerTransformers:
    ...
    def print_to_logging(
        ...
        return self.integration.transform_file(
            file_path, selector, print_modifier, dry_run
        )
    ...
    def format_to_fstring(self, file_path: Path, dry_run: bool = True) -> bool:
        ...
        return self.integration.transform_file(
            file_path, selector, format_modifier, dry_run
        )

    def update_deprecated_apis(
        self, file_path: Path, dry_run: bool = True, api_mapping: Dict[str, str] = None
    ) -> bool:
        """
    ...
    def optimize_imports(self, file_path: Path, dry_run: bool = True) -> bool:
        ...
        for selector in selectors:
            if not self.integration.transform_file(
                file_path, selector, import_modifier, dry_run
            ):
                success = False
    ...
    def python2_to_python3(self, file_path: Path, dry_run: bool = True) -> bool:
        ...
        for selector, modifier in transformations:
            if not self.integration.transform_file(
                file_path, selector, modifier, dry_run
            ):
                success = False
