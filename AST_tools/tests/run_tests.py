...
def copy_to_clipboard(text: str):
    ...
    try:
        if sys.platform == "win32":
            subprocess.run(["clip"], input=text.encode('utf-8'), check=True, stderr=subprocess.DEVNULL)
...
def run_suite():
    ...
    modules_to_test = [p for p in base_path.rglob('*.py') if '__init__' not in p.name and 'run_tests' not in p.name]
    all_results = []
